...
| Mitgliedsguthaben aufladen | `TransactionType.RECHARGE` mit `member_id` | Erhöht **Bar-Einnahmen** und den Soll-Kassenbestand um den Aufladebetrag; in der **aktuellen Systemstatistik** wird der Betrag zusätzlich im Gesamtwert der Buchungen und als `Guthaben verkauft` ausgewiesen | Erhöht Mitgliedsguthaben, Beleg in Transaktionsliste |
