    ...
    <div class="settings-grid">
      ...
      <div class="settings-card">
        <div class="settings-card-header">
          <div class="settings-card-icon">🔐</div>
          <div>
            <h3>Direktanmeldung "Kasse"</h3>
            <p>Erzwingt bei Deaktivierung die persönliche Anmeldung über individuelle Benutzerkonten.</p>
          </div>
        </div>
        <div class="session-timer-settings">
          <button
            class="btn"
            :class="kasseDirectLoginEnabled ? 'btn-success' : 'btn-secondary'"
            type="button"
            @click="kasseDirectLoginEnabled = !kasseDirectLoginEnabled"
          >
            {{ kasseDirectLoginEnabled ? 'Kasse-Direktlogin erlaubt' : 'Kasse-Direktlogin gesperrt' }}
          </button>

          <p class="text-hint">
            {{ kasseDirectLoginEnabled
              ? 'Der Button "Kasse anmelden" bleibt im Login sichtbar.'
              : 'Der Button "Kasse anmelden" wird im Login ausgeblendet und die API-Anmeldung als "Kasse" blockiert.' }}
          </p>

          <button class="btn btn-primary" type="button" @click="saveKasseDirectLoginSetting">
...
const saveKasseDirectLoginSetting = async () => {
  try {
    await appSettingsStore.saveAdminSettings({
      kasse_direct_login_enabled: kasseDirectLoginEnabled.value,
    })
    syncKasseDirectLogin()
    notificationStore.success('Direktanmeldung "Kasse" gespeichert')
  } catch (error) {
    notificationStore.error(error.response?.data?.detail || 'Einstellung konnte nicht gespeichert werden')
  }
}
