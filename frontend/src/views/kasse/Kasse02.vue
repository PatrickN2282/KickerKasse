        ...
        <div class="category-accordion">
          ...
          >
            ...
            >
              ...
              <span class="accordion-name">{{ category.name }}</span>
              <span class="accordion-count">{{ getProductsByCategory(category.id).length }}</span>
            </button>
          ...
          <!-- Ohne Kategorie -->
          <div v-if="productsWithoutCategory.length > 0" class="accordion-block">
            <button
              class="accordion-header"
              :class="{ open: expandedCategories.includes(0) }"
              @click="toggleCategory(0)"
            >
              <span class="accordion-chevron">▶</span>
              <span class="accordion-name">Ohne Kategorie</span>
              <span class="accordion-count">{{ productsWithoutCategory.length }}</span>
            </button>
...
.category-accordion {
  ...
  flex: 1;
  min-height: 0;
  padding-right: 2px; /* avoid clipping scroll */
}

.accordion-block {
  border: 1px solid color-mix(in srgb, var(--app-banner-color) 20%, transparent);
  border-radius: 10px;
  overflow: hidden;
  flex-shrink: 0;
}
...
.accordion-header {
  ...
  &:hover {
    background: color-mix(in srgb, var(--app-banner-color) 12%, var(--app-surface-color) 88%);
  }

  &.open {
    background: color-mix(in srgb, var(--app-banner-color) 10%, var(--app-surface-color) 90%);
  ...
  .accordion-chevron {
    ...
    border-radius: 4px;
    background: color-mix(in srgb, var(--app-banner-color) 14%, transparent);
    font-size: 9px;
    flex-shrink: 0;
  ...
  .accordion-count {
    ...
    padding: 1px 7px;
    border-radius: 999px;
    border: 1px solid color-mix(in srgb, var(--app-banner-color) 18%, transparent);
  }
}
...
.products-grid {
  ...
  gap: 6px;
  padding: 8px;
  background: color-mix(in srgb, var(--app-banner-color) 3%, var(--app-surface-color) 97%);
  align-content: start;
...
/* ── Product card (pos-07 dense style) ───────────────── */
.product-btn {
  background: var(--app-surface-color);
  border: 1px solid color-mix(in srgb, var(--app-banner-color) 20%, transparent);
  border-radius: 8px;
  ...
  &:hover:not(:disabled) {
    ...
    box-shadow: 0 3px 12px color-mix(in srgb, var(--app-highlight-color) 22%, transparent);
  ...
  .card-img {
    height: 72px;
    overflow: hidden;
    background: color-mix(in srgb, var(--app-banner-color) 6%, #f4f6fa);
    flex-shrink: 0;
    position: relative;
  ...
  /* Rabatt badge – top-left over image */
  .card-badge {
    position: absolute;
    top: 4px;
    left: 4px;
